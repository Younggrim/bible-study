...
# Each book: (folder name, book name for API query, chapter count)
books = [
    # Old Testament
...
for (folder, book_name, num_chapters) in books:
    ...
    for ch in range(1, num_chapters + 1):
        ...
        try:
            ...
            if not text:
                raise Exception("Empty passage returned")

            # Clean up ESV formatting — normalize whitespace
            import re
            # Replace verse number brackets [1] with "1." format
            text = re.sub(r'\[(\d+)\]', r'\1.', text)
            # Collapse multiple spaces/newlines between verses into single newlines
            text = re.sub(r'\n{2,}', '\n', text)
            text = re.sub(r'  +', ' ', text)
            text = text.strip()
